...
while true; do
    ...
    if [ $yn = "Y" ]
      then
        ...
        helm -n $NS install postgres-init-mockidentitysystem tf-govstack/postgres-init --set image.repository=tfgovstackdev/postgres-init --set image.tag=tf-develop -f init_values.yaml \
